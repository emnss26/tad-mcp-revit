...
from agent.core.planner import ensure_loaded, make_plan, model_info
...
def _filtered_actions():
    ...
    for name, model in ACTION_SCHEMAS.items():
        ...
        for arg_name, finfo in fields.items():
            try:
                is_req = finfo.is_required()  # pydantic v2
            except Exception:
                is_req = getattr(finfo, "default", Ellipsis) is Ellipsis
            (req if is_req else opt).append(arg_name)
        out.append({"action": name, "args": {"required": sorted(req), "optional": sorted(opt)}})
